...
raw_data= data.loc[(data["SvsN"]<10)]
raw_data_nnvt = data.loc[(data["MCP_Hama"] == 1) & (data["SvsN"]<10) ]
raw_data_hpk = data.loc[ (data["MCP_Hama"] == 0) & (data["SvsN"]<10)]
raw_data_nnvt_High = data.loc[(data["MCP_Hama"] == 1) & (data["HiQE_MCP"] == 1) &(data["SvsN"]<30) ]
raw_data_nnvt_Low = data.loc[(data["MCP_Hama"] == 1) & (data["HiQE_MCP"] == 0) & (data["SvsN"]<30)]

pv_tested_all=1.0/ raw_data["SvsN"]
pv_tested_nnvt=1.0 /raw_data_nnvt["SvsN"]
pv_tested_hpk=1.0/ raw_data_hpk["SvsN"]
pv_tested_nnvt_high=1.0/raw_data_nnvt_High["SvsN"]
pv_tested_nnvt_low=1.0/raw_data_nnvt_Low["SvsN"]
...
plt.savefig('SN_Tested.jpg',dpi=300)
plt.show()
